...
static void* runServiceThread(void *ptr)
{
    ...
    listen(sockfd,5);
    fprintf(stderr, "Service thread started\n");
    ...
    fprintf(stderr, "ScratchJr client connected\n");
    while(1) {
        bzero(buffer,256);
...
static void create_services(AvahiClient *c) {
    ...
    if (avahi_entry_group_is_empty(group)) {
        ...
        if ((ret = avahi_entry_group_add_service(group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, name, "_http._tcp", NULL, NULL, 11112, "test=blah", r, NULL)) < 0) {
...
int main(AVAHI_GCC_UNUSED int argc, AVAHI_GCC_UNUSED char*argv[]) {
    ...
    ret = pthread_create(&thread, NULL, runServiceThread, NULL);
    if(ret) {
    ...
    name = avahi_strdup("ScratchJrRover");
